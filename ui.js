(() => {
  ...
  const { $, $$, Type, Fields, SubSkills, Natures, NatureMeta, NatureUpGroups, NatureDownGroups, ExEffectLabels, PokemonList, MAX_SLOTS } = PS;
  ...
  function updateNatureOptions(slotIndex) {
    ...
    if (oldValue && candidates.some(([k]) => k === oldValue)) natureSelect.value = oldValue;
  }
  ...
  function applyNatureFromKey(slotIndex, natureKey) {
    ...
    const downSelect = $(`slot-${slotIndex}-nature-down`);
    const natureSelect = $(`slot-${slotIndex}-nature`);
    if (!upSelect || !downSelect || !natureSelect || !natureKey) return;
    ...
    if (opt) natureSelect.value = natureKey;
  }
  ...
  function getNatureEffectDescription(natureName) {
    ...
    const meta = NatureMeta[natureKey];
    if (!meta || (meta.up === "none" && meta.down === "none")) return "補正なし";

    const findLabel = (id, groups) => {
      const g = groups.find((x) => x.id === id);
      return g ? g.label.replace(/[▲▼]+/g, "").trim() : "";
    };
  ...
  function clearPokemonSlotSettings(slotIndex) {
    ...
    if (natureSelect) {
      updateNatureOptions(slotIndex);
      if (natureSelect.querySelector('option[value="Hardy"]')) natureSelect.value = "Hardy";
    }
  ...
  function collectSlotPokemonConfig(slotIndex) {
    ...
    return { pokemonKey, pokemonName, level, skillLevel, natureKey, subskills, personal };
  }
  ...
  function applyPresetToSlot(slotIndex, preset) {
    ...
    if (preset.natureKey) applyNatureFromKey(slotIndex, preset.natureKey);
  ...
  function renderPokemonPresetList(slotIndex) {
    ...
    presets.forEach((p, idx) => {
      ...
      let natureText = "";
      if (p.natureKey && Natures[p.natureKey]) natureText = `性格: ${Natures[p.natureKey].name}`;
      else if (p.natureKey) natureText = `性格: ${p.natureKey}`;
  ...
  PS.ui.applyNatureFromKey = applyNatureFromKey;
